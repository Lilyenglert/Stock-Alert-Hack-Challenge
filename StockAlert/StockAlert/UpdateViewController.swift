...
class UpdateViewController: UIViewController  {
    ...
    init() {
        self.stock = Stock(companyName: "", stockTicker: "", notificationType: .above, notificationPrice: 0, newsSource: .Twitter)
        self.toggleFlag = 0
    ...
    override func viewDidLoad() {
        ...
        /*newsSourceTextView = UITextView()
        newsSourceTextView.translatesAutoresizingMaskIntoConstraints = false
        newsSourceTextView.text = "News Source: "
        newsSourceTextView.textAlignment = .right
        newsSourceTextView.isEditable = false
        newsSourceTextView.isScrollEnabled = false
        newsSourceTextView.font = UIFont.systemFont(ofSize: 16)
        newsSourceTextView.textColor = .black
        view.addSubview(newsSourceTextView)
        
        newsSourceTextField = UITextField()
        newsSourceTextField.translatesAutoresizingMaskIntoConstraints = false
        newsSourceTextField.text = ""
        newsSourceTextField.textAlignment = .left
        newsSourceTextField.font = UIFont.systemFont(ofSize: 16)
        newsSourceTextField.textColor = .black
        newsSourceTextField.backgroundColor = .lightGray
        newsSourceTextField.inputView = newsSourcePickerView
        view.addSubview(newsSourceTextField)
        
        newsSourcePickerView = UIPickerView()
        newsSourcePickerView.translatesAutoresizingMaskIntoConstraints = false
        newsSourcePickerView.frame = CGRect(x: 0, y: 0, width: self.view.bounds.width, height: 280.0)
        newsSourcePickerView.dataSource = self
        newsSourcePickerView.delegate = self
        view.addSubview(newsSourcePickerView)*/
    ...
    func setupConstraints() {
        ...
        /*NSLayoutConstraint.activate([
            newsSourceTextView.topAnchor.constraint(equalTo: notificationPriceTextView.bottomAnchor, constant: 10),
            newsSourceTextView.centerXAnchor.constraint(equalTo: view.centerXAnchor, constant: -90),
            newsSourceTextView.heightAnchor.constraint(equalToConstant: 30),
            newsSourceTextView.widthAnchor.constraint(equalToConstant: 200)
            ])
        
        NSLayoutConstraint.activate([
            newsSourceTextField.topAnchor.constraint(equalTo: notificationPriceTextView.bottomAnchor, constant: 10),
            newsSourceTextField.centerXAnchor.constraint(equalTo: view.centerXAnchor, constant: 90),
            newsSourceTextField.heightAnchor.constraint(equalToConstant: 30),
            newsSourceTextField.widthAnchor.constraint(equalToConstant: 150)
            ])*/
